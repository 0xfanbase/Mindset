...
**v1.23 changelog (from v1.22, live feature request, same session):** live feedback on v1.22
asked for a substantial redesign of the Weeks tab before it ships for real: combine the two
grids into one, add a hover/tap highlight per person, a "high-tech" entrance animation, a
bigger/bolder/italic percent treatment that "hits harder," a clearer legend, an overall "cyber"
feel, and — the explicit gate before merge — multiple independent UX and code audits.

- **Design direction requested and followed again, this round even more directive:** the owner
  asked specifically for Fable's continued involvement, so the same design-direction agent
  thread from v1.22 was resumed (not a fresh one) with the new brief plus explicit technical
  tensions to resolve, not just relayed verbatim: what "combine into one chart" means
  mathematically given the two people's ~57-week age gap, and that literal hover has no iOS
  touch equivalent despite the owner's exact phrasing ("a hover over button"). Full spec and
  rationale in `audits/decisions.md`; every color and week-count claim in it was independently
  recomputed against the real shipped code before being relied on, again all exact.
- **Combined-grid cell model:** row/col stay age-weeks-since-birth (unchanged from v1.22), now
  drawn as ONE canvas instead of two. For age-week `w`: `w < Jw` (both have lived it) draws a
  vertically split cell, pink left / blue right; `Jw <= w < Bw` (only B has, so far — B is
  always older, so there is structurally no "J-only" state, ever) draws full-width solid blue;
  `w >= Bw` draws full-width faint future. `Jw` and `Bw` themselves each get an outline marker
  (pink/blue respectively) instead of a fill, with a small static glow underneath echoing
  `figure.js`'s own offscreen-radial-sprite glow technique — "the moment being lived, glowing."
  Two canvases become one; canvas memory roughly halves.
- **Hover/tap focus, adapted for iOS rather than implemented literally:** the two big percent
  blocks are real `<button>`s (`aria-pressed`, a 2px underline in person color when active —
  not color-only). Click/tap always toggles a sticky per-person highlight that pales the other
  person's marks to 25% opacity (geometry never changes, only ink). Real mouse `mouseenter`/
  `mouseleave` ALSO gives a live desktop preview, but only when
  `matchMedia("(hover: hover) and (pointer: fine)")` matches at setup time — binding those
  listeners unconditionally would fire `mouseenter` on iOS's first tap (WebKit's synthetic-
  hover-before-click behavior), turning one clean tap-to-toggle into a broken tap-to-preview/
  tap-to-toggle two-step on this app's stated primary platform. Verified via touch-emulated
  Playwright taps (one tap toggles, immediately) and separately via real mouse hover on a
  desktop-sized context (previews without touching `aria-pressed`, clears on mouse-leave).
- **Entrance animation:** the grid draws once, instantly, as before — no per-cell JS animation,
  ~9,000 cells redrawn on a timer would be both slower and worse-looking. Presentation is
  revealed via a 1100ms `cubic-bezier(0.16,1,0.3,1)` (the app's own house easing, already used
  for `cardIn`) top-down `clip-path` sweep with a synced scanline riding the reveal edge —
  GPU-composited, replays for free on every tab visit via the same `display:none` animation-
  restart the app already relies on for `.card`/`.value-row` (v1.16). `prefers-reduced-motion`
  needed real care, not just `animation:none`: the sweep's `from` state is fully clipped/
  invisible, so the override (added, correctly, at the stylesheet's last position per the v1.18
  cascade lesson) also sets `clip-path:none` explicitly rather than freezing on an invisible
  first frame, which would have been a worse regression than no animation at all.
- **Percent restyle:** Fraunces italic, weight 600, 34px, in the person's own fixed identity
  color, two side by side above the grid — the app's existing "artistic" typographic voice
  (already the wordmark's and Word of the Day's headline's) at real size, not a bigger mono
  digit, which would have read as a dashboard rather than this app's own instrument-panel
  register. Both colors independently reverified at this weight/size: they clear not just the
  3:1 large-text bar this size qualifies for but the full 4.5:1 normal-text bar on every
  background the app can show, unchanged from v1.22's numbers since the hex values didn't
  change, just their use.
- **Legend, directly under the grid:** `lived by both` / `only B, so far` / `J now` / `B now` /
  `to come`, each with a matching swatch (a 50/50 gradient chip for the split state) — the
  chart's five visual states now have a plain-language key instead of relying on the caption
  alone, per the owner's "idiot proof and abundantly clear" ask. The caption itself shrank to
  just the Burkeman attribution now that the legend carries the explanatory load.
- **Two real, generalizable bugs found in `scripts/verify.mjs` itself** (not in the feature
  code) by the first pre-merge audit of v1.22, both fixed and independently mutation-tested
  before this redesign continued:
  1. `check()` stringified an async check's return value (`String(detail)`) at push time,
     before the promise it actually was could ever resolve — collapsing it to the literal text
     `"[object Promise]"`. The tail loop that exists specifically to re-await promise-returning
     checks duck-types via `typeof r.detail.then === "function"`, which a plain string never
     satisfies, so it silently never fired. Net effect: **9 async checks — including both new
     v1.22 checks — reported green regardless of what they actually asserted**, proven by
     deliberately breaking `LIFE_PEOPLE` to use a real name and non-month-precision date and
     watching `verify.mjs all` still claim 65/65. Fixed by storing the raw value instead of
     stringifying it (one line); reverified the fix itself by re-running the exact same
     mutation and confirming it now correctly reports the injected failures with the right
     messages, then confirming a clean run is genuinely 65/65 with the async bodies actually
     executing.
  2. Plain `node --check <path>.js` is unreliable in this repo specifically because there is no
     `package.json` to declare `"type":"module"` — its CommonJS/ESM sniffing can silently
     under-report a real syntax error once a file has a top-level `import`/`export` (confirmed
     by isolated repro: identical bytes correctly failed both `--check --input-type=module` and
     a real `import()`, but passed plain `--check <path>.js` clean). This masked two literal
     `#`-instead-of-`//` typos the implementer introduced while rewriting `weeks.js` for this
     redesign — caught only because the file was also sanity-checked via a real `import()`, not
     because `verify.mjs` caught it. Fixed with a shared `nodeCheckSyntax()` helper that pipes
     file content through stdin with `--input-type=module` (syntax-only, no top-level browser-
     global execution, so `app.js`/`figure.js`/`weeks.js` can be checked without a DOM); `.mjs`
     files were confirmed unaffected (extension alone is unambiguous) and left as-is.
  Per invariant 12 both are tightenings (the checks now verify what they always claimed to),
  logged here and in `audits/decisions.md` in full since they affect trust in every prior
  "verify.mjs all: N/N" claim in this file — none of those runs were dishonest, the harness
  itself just wasn't fully checking what it reported checking.
- **A real self-caught rendering bug:** the current-week glow, first tuned at figure.js-style
  0.55 peak alpha / 2.2x-pitch stamp size, was strong enough at that size to bleed a full extra
  cell into neighboring "both lived" split cells and to visibly refill the current-week
  outline's supposed-to-be-transparent center — undermining both the split-cell read and the
  "unfilled, not yet complete" meaning of the outline. Caught by the same pixel-sampling
  technique used to verify v1.22 (this time sampling all 5 cell states plus both focus states
  at their exact boundary indices, freshly recomputed for the new geometry rather than reusing
  v1.22's), not by eye. Retuned to 0.28 peak alpha / 1.15x-pitch — reverified pixel-exact:
  neighboring cells read their pure person color again, and the marker itself now carries a
  clearly-subordinate (~0.25-0.3 alpha, plainly distinct from a solid 1.0-alpha lived cell) glow
  rather than one that competed with the "unfilled" reading.
- **`redrawAll()` bails out (`chart.scroller.clientWidth === 0`) before touching
  `lastDrawnDateHKT` or the zoom-button state** — a fix for a second real bug the same v1.22
  audit found: the `ResizeObserver` added to catch orientation changes fires with a zero size
  while the Weeks panel is `display:none` (hidden tab), and would otherwise shrink the canvas
  to `MIN_PITCH` and stamp today's date as "already drawn," so the NEXT real (visible) tab
  activation would see an unchanged date and skip the redraw that should have restored the
  correct size — a visible mis-sized flicker on every tab round-trip. Also added `role="group"`
  to the scroller per the same audit's minor notes, and a targeted stage1 check scanning
  `weeks.js`'s `CAPTION`/legend-label string literals for quotation-mark glyphs, closing a gap
  the audit named explicitly (the existing scoped sweep only ever covered `cards.json`/
  `values.json`/`app.js`, never any other JS file's user-facing copy).
- **`sw.js`:** `CACHE` bumped `"mindset-v8"` → `"mindset-v9"` (Appendix C.2 updated to match) —
  `weeks.js`'s own content changed substantially.
- **`verify.mjs` tightened again, per the invariant-12 ratchet:** the two harness fixes above,
  plus a new pinned check that the B-minus-J age-week gap stays within `{56, 57}` (confirmed by
  an exhaustive 40-year daily sweep during implementation — the two birth months are a fixed
...
- **Verified pre-audit:** `verify.mjs all` 67/67, genuinely (not just nominally — see above).
  ...
  width tested, entrance animation verifiably starts clipped and settles unclipped, scanline
  fades to ~0, reduced-motion shows the full grid with no clip artifact, theme toggle leaves
...
- **Today's normal mode is now a LOGGED §4.4 one-screen exception, like Weeks,** not a silent
  ...
  v1.33 entry below. Superseded again, and resolved, by v1.35: retiring the fourth (Word of
  the Day) card brought the true worst case back to exactly 844px — no exception needed
  anymore. See the v1.35 entry.]**
...
**v1.35 changelog (from v1.34, live request):** owner: "Please remove the word of the day
block at the bottom of the first tab because that is not helpful. Kill it." Word of the Day
(added v1.10, reworked in v1.22) removed entirely, following the same full-removal pattern as
v1.31's Closing retirement rather than just hiding it:

- **`app.js`:** `renderWordCard`/`speak` (the pronunciation-button `SpeechSynthesisUtterance`
  wiring) deleted; the `word` destructure/guard/pick dropped from `renderToday`'s fresh,
  offline-fallback, and staleness-check paths; the WORD chip dropped from the four-card
  render list (now three: Journal, Anchor, Kenya).
- **`lib.mjs`:** `pickToday` no longer picks or returns a `word`.
- **`data/cards.json`:** the 30-entry `wordOfDay` array deleted outright (412,891 B total,
  down from 419,728 B — the pool itself was ~6.8KB). Historical content preserved,
  retired-labeled, in `audits/CONTENT-REVIEW.md` — which turned out to still be showing the
  superseded v1.10 draft pool (wabi-sabi/ikigai/...), never updated when v1.22 reworked it to
  the actually-shipped English-word/idiom pool; corrected to the real final content while
  writing this retirement note, not a new gap introduced by it.
- **`scripts/generate-daily.mjs`:** no longer picks or stamps `wordId`; the pipeline now
  stamps three ids, not four.
- **`data/daily.json`** regenerated — `wordId` gone, the other three ids unchanged (each
  pool's rotation is independent, so removing `word`'s salt doesn't touch anchor/journal/kenya).
- **`scripts/verify.mjs`:** the `wordOfDay`-shape/count/word-cap/quote-glyph/platitude/
  offline-fallback checks that iterated it had that iteration deleted outright (nothing left
  to check); the two shape assertions (`cards.json`'s `wordOfDay`, `daily.json`'s `wordId`)
  were inverted into `assert.equal(..., undefined, ...)` negative guards, same treatment
  Closing's got in v1.31, so an accidental reintroduction fails loudly. The `"word"` salt
  stays in the seam-gap sweep as a generic-correctness check, joining the already-retired
  `"closing"` salt there. Net check count: 86 → 85 (one check, "wordOfDay entries have
  word/origin/lang/meaning...", had nothing left to iterate and was deleted, not retargeted).
- **`styles.css`:** `.word-title-row`/`.word-title`/`.word-speak` (and its `:active`/
  `:focus-visible`/`:hover` states) deleted outright — nothing else referenced them. `.tab`
  gained `min-width: 44px` in v1.34 for unrelated reasons and is unaffected.
- **A genuine, unplanned side effect: the long-standing §4.4 one-screen exception for Today's
  normal mode is now fully resolved, not just re-measured.** That exception (logged since
  v1.28, re-measured in v1.33 when it grew to 1020px) existed because the four-card stack
  could run longer than 844px depending on the day's content. Re-measured live post-removal
  against the true worst case (longest anchor + longest journal + longest kenya, all three at
  once, forced via route interception): **exactly 844px** — the invariant now holds without
  exception for the first time since v1.28. `styles.css`'s comment and the v1.28 changelog
  entry above both updated to record this as resolved, not just re-superseded.
- **Docs swept for stale "Word of the Day"/"four cards" references** across `index.html`,
  `manifest.webmanifest`, `README.md`, and this file's live spec sections (§3 repo layout,
  §4.5 components, §5.1/§5.2/§5.3 content spec, §6.1/§6.2/§6.3 data & logic, §7 pipeline, §9.3
  loops) — retired-feature slots (item 4a, §5.3 rule 10) kept as retirement stubs rather than
  renumbered, so existing cross-references elsewhere in this file stay valid. Genuinely
  historical text (§1's Stage-0 mission summary, dated `**vX.Y changelog**` entries, the
  original Stage 0–5 build-task list) deliberately left alone, matching how this file has
  already treated Kenya/Weeks/Mara/Closing's own additions and removals throughout its
  history — those were never retrofitted into §1 or the Stage tasks either.
- **Budget impact: both ceilings gained headroom, no exception needed.** JS 63,921 B → 62,795
  B of 66,560 (65KB) — 3,765 B headroom, up from 2,639 B. Page weight 558,370 B → 549,538 B
  of 614,400 (600KB) — 64,862 B headroom, up from 56,030 B.
- **`sw.js` `CACHE` bumped `mindset-v18` → `mindset-v19`** (`app.js`/`lib.mjs`/`styles.css`
  bytes changed); Appendix C.2 updated to match.
- **Verified:** `verify.mjs all` 85/85 (86 → 85, the one genuine removal above). `node --check`
  clean on every touched `.js`/`.mjs` file. Live Playwright: three cards render (not four),
...
**v1.37 changelog (from v1.36, live feature request):** the owner asked for the Values tab to
be removed — "we rarely refer to it." Values has been part of this app since v1.0 (§1's original
"Two tabs: Today and Values" framing, values.json cut from 10 to 5 entries in v1.2), so unlike
Mara this touches content genuinely woven through the original spec, not a late addition — the
docs sweep below is correspondingly wider.

- **Removed outright:** `app.js`'s `renderValues`/`renderValuesError` and the `boot()` fetch of
  `data/values.json`; `index.html`'s `#tab-values`/`#panel-values` (nav button + tabpanel) —
  `role="tablist"` now holds two tabs, Today and Weeks; `styles.css`'s `.value-row`/`.value-name`/
  `.value-essence`/`.value-behaviour`/`.values-empty` rules (26 lines) and the retargeted reduced-
  motion/fill-mode selector lists that used to include `.value-row`; `data/values.json` itself
  (deleted, 779 bytes); `sw.js`'s `ASSETS` entry for it. Five surviving comments that cited
  Values as a worked example for unrelated rules (Today-card styling rationale, the Weeks
  overflow-y precedent, the v1.16 animation-restart note, the "kept last on purpose" cascade-order
  note, the tabs-array boolean-vs-array note) were reworded to drop the now-dead reference rather
  than left dangling — same treatment as the three Mara-citing comments in v1.36. `README.md`'s
  intro paragraph ("A quiet Values tab sits alongside it"), its "A third tab, Weeks" (now the
  *second* tab) line, and its Ops-runbook mention of editing `data/values.json` were all updated
  too — omitted from this entry's first draft, caught and added per the v1.37 audit below.
- **`verify.mjs`, net unchanged at 80 checks, tightened not loosened per the invariant-12
  ratchet:** unlike Mara's checks (all standalone), Values' checks were interleaved into shared
  ones — the dedicated `"data/values.json valid JSON, exactly 5 values"` check was deleted
  outright (1 check), and the values-scanning clauses inside the shared word-cap and quote-glyph
  checks (which also cover anchors/journal/kenya) were removed, narrowing those checks' scope
  without deleting them (0 checks). One new negative-guard check was added, mirroring the Mara/
  Closing/Word-of-Day precedent: `data/values.json` must not exist, so an accidental
  reintroduction (a bad merge, a stray revert) fails loudly. Net: 80 → 80 (1 removed, 1 added) —
  see `audits/decisions.md`'s v1.37 entry for the full accounting.
- **`BUILD-PLAN.md` docs swept for live-spec references, historical text left alone** (matching
  the v1.35/v1.36 precedent): §3 repo layout (`values.json` tree line removed), §4.3 typography
  (Fraunces role list and the type-scale line both dropped their values-specific entries), §4.5
  Components item 3 (Cards, which used to define its own styling by contrast with "the Values
  tab's quieter flat list" — reworded to stand on its own, same fix already made to its styles.css
  twin comment) and item 5 (Values tab) and §5.3 Writing Rules item 6 (Values schema) — item 5 and
  item 6 both converted to retirement stubs, slot kept, not renumbered, same pattern as v1.35's
  `item 4a`/`§5.3.10` for Word of the Day. Unlike item 4a, no live `§4.5.5` cross-reference was
  ever found to actually require this (checked directly: nothing in this file cites `§4.5.5` or
  `item 5`) — the slot is kept anyway for consistency with item 4a's established treatment, not
  because renumbering would break a real pointer. `§5.3.6` is different: §5.2's Claude-Design-
  prototype origin note genuinely cites it by number, so that slot's preservation is load-bearing,
  not just stylistic. §4.5 item 6 (Motion) and §6.3 item 1 (client fetch
  list) both dropped their Values-specific clauses. **Deliberately left alone, on the same basis
  established across every prior addition/removal (Kenya, Weeks, Mara-add, Mara-remove, Closing,
  Word of Day):** §1's mission summary (still says "Two tabs: Today and Values" — frozen v1.0
  text, never retrofitted for any of the last five tab/card changes either), the §4.4 ASCII
  one-screen mockup (still shows only "Today / Values"), the original Stage 0–5 build-task list
  (Stage 3/5's "real values"/"edit values" task prose), §13's human review checklist (item 5,
  "Open the Values tab" — already stale for Weeks/Mara/Kenya before this round, not newly broken
  by it), and Appendix A's `verify.mjs` requirements (already frozen at the original 350KB/60KB
  budget figures and the original three-data-file scope, predating every budget raise and content
  addition since — not just this one). All of these were re-confirmed un-retrofitted for Kenya/
  Weeks/Mara/Closing/Word-of-Day before this round, so leaving them for Values keeps the file
  internally consistent rather than fixing one stale reference while five others remain.
- **Budget impact, informational only — both ceilings gained headroom:** JS 51,633 B of 66,560
  (65KB) — up from 14,013 B headroom (post-v1.36) to 14,927 B. Page weight 487,801 B of 614,400
  (600KB) — up from 123,862 B headroom to 126,599 B.
- **`sw.js` `CACHE` bumped `mindset-v20` → `mindset-v21`** (`data/values.json` dropped from
  `ASSETS` entirely); Appendix C.2 updated to match.
- **Verified:** `verify.mjs all` 80/80. `node --check` clean on `app.js`/`sw.js`/
  `scripts/verify.mjs`. A repo-wide case-insensitive grep for `values` after all edits returns
  zero hits outside historical changelog/audit prose, the sections listed as deliberately-left-
  alone above, and unrelated uses of the word ("CSS property value," "hex values," "wordId
  values," `Object.values()`). Live Playwright pass at 390×844 (routed straight from disk, same
  method as v1.36): **11/11** — exactly 2 tabs (Today/Weeks, no `#tab-values`/`#panel-values`
  anywhere in the DOM), 3 Today cards render and settle to full opacity, one-screen layout holds
  (`scrollHeight` 844), both tabs clear the 44px tap-target floor, arrow-key roving focus wraps
...
**v1.39 changelog (from v1.38, live feature request, Opus as project director and auditor):**
the owner asked for four things in one request — cut out the Anchor card, remove the morning
(pre-09:00 HKT) hiding function, merge the Weeks content into a new single-page view, then
remove the Weeks tab since it wasn't merged into that single page — with Opus explicitly
directing the plan up front and auditing the result afterward. Unlike Mara/Values/Kenya
(clean feature removals) and unlike focus mode alone (a UI-state removal), this round is a
genuine architectural restructuring: the tab system itself is retired, `<main>` changes layout
mode (flex column → block-flow scroll container), two previously-independent render
lifecycles (Today's card render, Weeks' lazy tab-activation build) merge into one boot
sequence, and invariant 11's one-screen guarantee is re-scoped rather than simply dropped.

- **Removed outright:** `app.js`'s `initTabs`, `renderAnchorCard`, `paintFocusedToday`,
  `windowMode`/`paintedWindowMode`, and the `anchor`/`winMode` branches inside `renderToday` —
  not just deleted calls, but every conditional path that touched them, confirmed by re-reading
  the whole file after editing rather than trusting a partial match (the exact discipline the
  v1.38 audit's self-contradiction finding exists to enforce). `lib.mjs`'s `isFocusWindowHKT`
  and the `anchor` pick inside `pickToday` (which now returns `{journal, dayNumber}`, not a
  second field nobody reads since Mara/Values/Kenya never depended on it either).
  `scripts/generate-daily.mjs`'s `anchorId` stamp — `data/daily.json` regenerated for real via
  the actual pipeline (`NODE_USE_ENV_PROXY=1 node scripts/generate-daily.mjs`), not
  hand-edited; today's `journalId` (`journal-1013`) came back byte-identical to its
  pre-regeneration value, directly confirming Anchor's removal didn't perturb Journal's
  independent rotation salt. `data/cards.json`'s 365-entry `anchors` array (removed via a small
  Node script that parsed, deleted the key, and re-serialized — not hand-edited text surgery on
  a 399KB file —399,031 -> 290,762 bytes, a much larger single-round content removal than any
  prior round's, since Anchor's pool was 6x Kenya's). `index.html`'s `role="tablist"` nav and
  both `role="tabpanel"` wrappers, replaced by a flat `#cards` / `.section-divider` /
  `#weeks-root` sequence directly inside `<main>`. `styles.css`'s `.tabs`/`.tab` rules, `.panel`
  rules (their `overflow-y`/padding absorbed into `main` itself, which drops `display:flex` for
  plain block flow — the simpler shape for a container that now just stacks three siblings.
  **This entry's original rationale — that a flex column would let `overflow-y:auto`
  shrink-to-fit its children instead of overflowing, per `flex-shrink`'s default — was wrong;
  the v1.39 audit measured the counterfactual instead of re-reasoning it and found the two
  layouts identical. See the audit note at the end of this entry.**),
  `.reveal-rest`/`#cards-more` rules, the desktop `#cards` flex-row/`.focus` override (replaced with `#cards`
  joining the existing 640px-centered rule alongside `.weeks-stats`/`.weeks-zoom-row`/
  `.weeks-card`, since a single card stretching to `main`'s full width no longer makes sense),
  and the now-fully-dead `.card-attr` rule (Anchor's attribution line was its only consumer;
  Journal never had one). `weeks.js`'s `initWeeksTab()` renamed to `initWeeks()`, called once
  from `app.js`'s `boot()` (wrapped in its own try/catch so a Weeks failure can't take down
  Journal's render) instead of on first tab activation; its zero-width bail-out guard in
  `redrawAll()` is kept verbatim as a general defensive check, no longer the tab-activation
  guard it used to be. Eighteen comments across `app.js`/`lib.mjs`/`weeks.js`/`styles.css` that
  cited tabs/focus-mode/Anchor as a design precedent or worked example for unrelated code were
  reworded rather than left dangling (3 in `app.js`, 1 in `lib.mjs`, 6 in `weeks.js`, 8 in
  `styles.css`; this entry first said "ten," recounted from the diff by the v1.39 audit — the
  same undercount-of-reworded-comments slip the v1.38 audit caught a round earlier).
- **`verify.mjs` — invariant-12 ratchet, 77 -> 76 (2 removed, 1 added, 9 retargeted):** the
  anchor category-counts check (`CATEGORY_COUNTS` constant removed too, now fully dead) and the
  anchors-only near-duplicate-proxy check had nothing left to check once their subject was gone
  and were deleted outright. One new check was added: a structural assertion that `#cards`,
  `.section-divider`, and `#weeks-root` appear in that DOM order inside `<main>` (the mechanical
  proof that the single-page merge actually happened, not just that the tab markup is gone).
  Nine checks were retargeted rather than removed or added (this entry first said "six," which
  counted the six clauses below rather than the nine individual `check()` bodies they name —
  recounted against the diff by the v1.39 audit): the tablist/tab/aria-selected check
  flipped from asserting PRESENCE to asserting ABSENCE (same treatment as the Mara/Values
  retired-file guards); `isFocusWindowHKT`'s boundary-pinning check became a retirement guard
  (mirroring `isEveningWindowHKT`'s own v1.31 retirement-guard shape) that also asserts
  `renderAnchorCard`/`paintFocusedToday`/`windowMode` never reappear in `app.js`; the
  `cards.json` shape check gained an inline `anchors: undefined` guard (the Closing/Word-of-Day/
  Kenya precedent — Anchor was a pool *inside* a shared file, not its own file, so it gets a
  folded-in guard, not a dedicated `check()` call the way Mara's/Values' file-existence guards
  were); the word-cap/quote-glyph/platitude checks dropped their anchors-scanning clauses
  without narrowing journal's coverage; the rotation and offline-fallback simulation checks
  retargeted from `anchors`/`"anchor"` to `journal`/`"journal"`; the `daily.json` schema check
  gained an inline `anchorId: undefined` guard. Left alone, per established precedent: the
  `pickIndex` full-cycle uniqueness check's use of `"anchor"` as a generic salt (it's testing
  the rotation primitive itself, not asserting Anchor-the-feature exists), the seam-gap sweep's
  salt array (which keeps `"anchor"` forever alongside `"word"`/`"closing"`/`"kenya"`, proving
  the guarantee isn't accidentally salt-specific), and the generic "all ids unique per pool"
  check (already pool-agnostic). Full accounting in `audits/decisions.md`'s v1.39 entry.
- **`CLAUDE.md`/`BUILD-PLAN.md` invariant 11 re-scoped, not silently narrowed — a first for
  this session (three prior rounds never touched CLAUDE.md itself):** the one-screen guarantee
  now covers "the day's Journal card complete above the fold," not the whole page — Weeks below
  the section divider scrolls, by design. Logged as its own dated `decisions.md` sub-entry
  (quoting the original invariant-11 wording) per invariant 12's rule that an invariant TEXT
  change itself gets logged, distinct from a `verify.mjs` check change.
- **`BUILD-PLAN.md` docs swept for live-spec references, historical text left alone (matching
  the Mara/Values/Kenya precedent), wider than any prior round's sweep given the scale of the
  change:** title bump to v1.39; §2 invariant 1's now-moot present-tense "is the point of the
  Anchor cards" reworded to past tense; §2.11's mobile-first law re-scoped alongside
  `CLAUDE.md`'s; §3 repo tree (`app.js`/`weeks.js` comments, `cards.json` line); §4.1's
  one-sentence direction (dropped its own "fits without scrolling" claim); §4.2 confirmed
  unaffected (pure color tokens, no anchor/tab content) and left as-is; §4.3 typography (dropped
  its anchor-specific type-scale entry, and while already in that exact line, fixed a
  pre-existing miss unrelated to this round: "20 (word title)" had survived un-caught since
  Word of the Day's v1.35 retirement); §4.4's heading and both its intro/post-mockup paragraphs
  reframed around "Journal fits, Weeks scrolls" — the ASCII mockup itself stays frozen, per the
  now four-round-strong precedent that it has never once been kept in sync with any tab or card
  change; §4.5 items 3 (Cards, rewritten for exactly one card) and 4b (focus mode, converted to
  a retirement stub, slot kept) and 6 (Motion, reworded for a single card) and a genuinely new
  item 7 (Weeks joining the single page — the first live §4.5 description Weeks has ever had,
  since it was previously documented only in its own changelog entries); §5.1 schema example;
  §5.2's count table (11 anchor rows collapsed into one retirement-note parenthetical, matching
  the wordOfDay/kenya precedent exactly); §5.3 rules 1/2 edited in place (their word-cap/
  attribution language was quietly Anchor-shaped; now generic, pointing to rule 5 for Journal's
  actual numbers) plus rules 7/8/9 converted to retirement stubs (attribution-confidence,
  anchor-authoring-process, and the `voices` sub-category — all three exclusively about
  Anchor) and a new rule 12 summarizing the section's now-fairly-complex stub/edited-in-place
  structure for a future reader; §6.1 schema example and its trailing note; §6.2's salts list
  and `minSeamGap` worked-example numbers; §6.3 confirmed unaffected (describes the fetch/
  staleness contract generically) and left as-is; §7's pipeline description, item 1, and a new
  v1.39 note mirroring v1.35's; §9.3's LOOP-D row; §13 items 1 (rewrote the one-screen claim to
  describe the scroll-into-Weeks behavior) and 6 ("two cards" -> "card"); Appendix C.1's
  description string and Appendix C.2's `CACHE` bump plus, while that exact code block was
  already open, a resync of its long-drifted `ASSETS` array literal (missing `weeks.js`/
  `manifest.webmanifest`/fonts, still listing the v1.37-retired `data/values.json` — harmless
  since `verify.mjs`'s byte-identity check is explicitly modulo this array, fixed anyway now
  that the block was open for other reasons). **Deliberately left alone, on the same basis
  established across every prior addition/removal:** §1's mission summary (still says "Two
  tabs: Today and Values" and lists Anchor as card 1 of three — frozen v1.0 text, never
  retrofitted for any of the last six tab/card changes either), the §4.4 ASCII mockup itself,
  §4.7 item 3 (its "(§4.4/invariant 11)" pointer stays accurate once both targets are updated —
  a pointer doesn't need to restate what it points to), the original Stage 0-5 build-task list
  (Stage 1's "matching the one-screen layout," Stage 4's Verify line still naming
  `anchorId`/`kenyaId`/`wordId`), §5.2's closing paragraph (the Claude-Design-prototype seeding
  narrative, which has already survived the shifts->journal and freshReserve->wordOfDay renames
  untouched, and §5.3 rule 5's "matching the anchors precedent" clause (historical grounding
  for journal's own sizing choice, still accurate as history), §12, §13 item 5 ("Open the
  Values tab" — already stale before this round, not newly broken by it), and Appendix A
  (still frozen at the original budget figures, file scope, and three-pool description,
  predating every round since v1.0).
- **Budget impact, informational only — both ceilings gained the largest single-round headroom
  jump of any prior round, since Anchor's 365-entry pool dwarfed every previously-retired
  pool:** JS 46,811 B of 66,560 (65KB) — up from 16,744 B headroom (post-v1.38) to 19,749 B.
  Page weight 358,392 B of 614,400 (600KB) — up from 142,735 B headroom to 256,008 B (the
  108,269-byte `cards.json` shrink alone accounts for most of the jump). No exception needed or
  taken.
- **`sw.js` `CACHE` bumped `mindset-v22` -> `mindset-v23`** (no separate file dropped from
  `ASSETS` this time — `app.js`/`lib.mjs`/`styles.css`/`index.html`/`data/cards.json`/
  `data/daily.json` all changed bytes in place); Appendix C.2 updated to match, including the
  incidental `ASSETS`-array resync noted above.
- **Verified:** `verify.mjs all` 76/76. `node --check` clean on `app.js`, `lib.mjs`, `weeks.js`,
  `scripts/generate-daily.mjs`, `scripts/verify.mjs`. A repo-wide case-insensitive grep for
  stale tab/focus-mode/Anchor references after all edits returns zero hits outside historical
  changelog/audit prose and the sections listed as deliberately-left-alone above. Live
  Playwright, 8 scenarios at 390×844 (routed from disk, no dev server), each clock-mocked to a
  specific HKT instant, **39/39 assertions green**: normal midday (14:00 HKT) -- zero
  tablist/tab/tabpanel roles anywhere in the DOM, exactly 1 card (JOURNAL), no
  `.reveal-rest`/`#cards-more` anywhere, `#cards` -> `.section-divider` -> `#weeks-root` in
  that DOM order, Weeks built at boot with a non-zero-width canvas, `main` computes
  `display:block`/`overflow-y:auto`, the Journal card's bottom edge sits at 227px (well inside
  844px, no scroll needed), the full page (1163px) exceeds the viewport and scrolling genuinely
  reveals Weeks; worst case (`journal-0359`, the longest of all 1825 entries, forced via
  `data/daily.json` route interception) -- still 251px, comfortably above the fold; 04:30 HKT
  (previously inside the old focus window) -- renders identically to any other hour, no reveal
  toggle, confirming focus mode is truly gone and not just coincidentally absent at that hour;
  offline fallback -- slate chip, valid rotated content; 20:00 HKT dark theme -- renders
  cleanly; Weeks interactivity -- zoom and the J/B stat toggle both work with Weeks built
  eagerly rather than lazily; keyboard focus order -- theme toggle then straight into Weeks'
  own controls, no orphaned tab-panel remnants; reduced motion -- both the card entrance and
  the Weeks clip-reveal are suppressed, canvas fully visible (not stuck clipped). Zero
  console/page errors in every scenario.
- **The independent v1.39 audit (Opus, fresh context, worktree-isolated) found no functional
  defect** — the architectural risk this round was specifically asked to guard against checked
  out clean across 53 additional live-browser assertions in 13 clock-mocked scenarios (plus a
  no-assertion flex-vs-block control), covering ground beyond this entry's own eight: a
  stale-`daily.json` amber path, an unresolvable `journalId`, a legacy `daily.json` still
  carrying `anchorId`, a legacy `cards.json` still carrying a stray `anchors` pool, 08:30 HKT,
  and a 1100px desktop pass. Three further out-of-suite probes ran alongside it: a
  service-worker isolation check (see the console-error caveat in `decisions.md`), a
  five-viewport flex-vs-block comparison, and a fault-injection pass replacing `initWeeks()`
  with a throwing stub — which confirmed `boot()`'s new try/catch genuinely keeps the Journal
  card rendering, the one behavior that guard exists for and the one this round never tested.
  Every byte-count, the check-count reconciliation, and the Appendix C.2 `ASSETS` resync were
  independently recomputed and all matched exactly; `sw.js` is byte-for-byte identical to the
  Appendix C.2 block including the array. It did find one wrong technical claim and a cluster
  of documentation slips, all fixed above and below rather than left standing:
  - **The `main` flex-to-block rationale was wrong, and was asserted as independently
    re-derived and browser-verified.** Measured directly by rendering the shipped page against
    a variant that re-adds `display:flex; flex-direction:column` to `main`: geometry is
    byte-identical at 390x844, 390x500, 320x400, 390x2000 and 1100x900. Two independent
    reasons it cannot differ: `body`'s `min-height:100svh` + auto height means it grows past
    the viewport, so `main` (`flex:1`) is never height-constrained —
    `main.scrollHeight === main.clientHeight` on every load, `main`'s `overflow-y` never
    engages, and the DOCUMENT is the scroll container (the same app-wide property the v1.25
    audit already recorded about `.panel`); and even under a constrained height, a column flex
    item's automatic minimum size (`min-height:auto`) holds auto-height children like these at
    content height — verified in an isolated DOM probe where auto-height children gave
    identical results in both modes and only explicit-height children shrank. The change is
    harmless and arguably cleaner, so it stands; only its stated justification was corrected,
    in this entry, `styles.css`, and `decisions.md`.
  - **A self-contradiction of exactly the v1.38 shape, in this round's own
    self-contradiction check:** `decisions.md` claimed a post-edit grep confirmed §4.5 item 3
    "genuinely does NOT point at item 4b anymore." Item 3 points at item 4b twice, in text
    written this round. Item 4b's stub itself never made a false claim (it claimed nothing),
    but it understated its slot as kept "for consistency" when the slot is in fact
    load-bearing — corrected to match items 4a/4c's language, and the `decisions.md` claim
    corrected too. §5.3 rule 8 got the same correction for the same reason (three live
    pointers, all written by this round's own sweep: item 3's "see item 8 in §5.3," and rules
    1 and 2's "see rule 8"). Rules 7 and 9 were checked and left as consistency-only slots —
    their pointers exist but sit in frozen historical text (Stage 3's task list, the v1.3
    changelog), not live spec prose. §4.5 item 5's "no live `§4.5.5` cross-reference" claim
    was re-checked after all edits and still holds.
  - **Three stale/mis-stated numbers, all corrected in place:** §4.5 item 4 still said the
    offline path "computes both cards" — the exact line the v1.38 audit had corrected from
    "all three cards" one round earlier; §6.1's retirement note had `wordId`'s ordinal changed
    from "fourth" to "second," which left two different ids both called the second; and the
    reworded-comment count ("ten") and retargeted-check count ("six") were both undercounts of
    the diff (18 and 9).
  - **Two dangling references the round's own sweep missed:** `styles.css`'s epigraph comment
    still cited `.card-attr`, a rule deleted in this same commit, with no retired marker; and
    `verify.mjs`'s seam-gap comment enumerated every retired salt except `"anchor"`, the one
    retired this round.
  - **`audits/CONTENT-REVIEW.md` self-contradicted across six lines:** the new v1.39 addendum
    says the anchors pool is "no longer current or shipping," while the "Anchors v1.0–v1.13"
    section immediately below still pointed at the v1.14 section as "the current, shipping
    content." Corrected. (The v1.39 retitle itself was verified to have landed on the correct
    section — the live v1.14 pool at the file's third "Anchors"-named heading, not the
    already-historical pre-v1.14 one.)
  - **Flagged, deliberately not changed — an owner copy call, not a defect:** the app's own
    user-facing plural copy now reads oddly with exactly one card — the error card's
    "Couldn't load today's cards," the amber `yesterday's cards` chip, and `README.md`'s
    matching prose. All three are shipped UI strings, unchanged since v1.0 and left alone by
    v1.35/v1.36/v1.38's own removals; renaming them is a design decision, not a docs sweep.
    Separately noted, pre-existing and out of this round's scope: `audits/build-log.md`'s last
    heartbeat is v1.27, stale across every round since.

**v3.0 changelog (from v2.0 — note v2.0, the 2026-08-24 UI redesign, was logged only in
audits/decisions.md and never received a BUILD-PLAN changelog entry or title bump; this entry
covers both):** owner request (verbatim): "I now just want the 4000 weeks screen without the
top journal ... 4000 weeks become the centre piece of the site for us ... re-audit and rework
the entire stack." The Journal card is retired, and with it everything that existed only to
feed it: the daily pipeline (`scripts/generate-daily.mjs`, `data/daily.json`,
`.github/workflows/daily.yml`), the watchdog (`.github/workflows/watchdog.yml` — it only ever
checked `daily.json` staleness), the content library (`data/cards.json`, 1825 journal prompts —
historical content survives in git history, at the commit immediately before this round's, and
in `audits/CONTENT-REVIEW.md`, which keeps a retirement note), the rotation engine in `lib.mjs`
(`pickIndex`, `xmur3`, `mulberry32`, `shuffledOrder`, `shuffledOrderSeamSafe`, `minSeamGap`,
`pickToday`), the staleness model (`staleness`, `expectedDateHKT`, the chip), and the `data/`
directory itself. What stays: the header (wordmark, HKT date line, breathing figure
`figure.js`, theme toggle), the two-theme clock model, the Weeks section (heading, per-person
stat buttons with progress bars, zoom, canvas grid, epigraph), PWA installability, the footer,
and `pages-deploy.yml` (on push, unchanged in trigger). Weeks is now the entire page content,
computed from today's HKT date on every load and on every `visibilitychange` resume — nothing
refreshes on a schedule anymore; the site only changes when a human pushes to `main`.
...
- **`sw.js` `CACHE` bumped `mindset-v23` -> `mindset-v24`** (`ASSETS` drops
  `./data/cards.json`/`./data/daily.json`, since `data/` no longer exists); Appendix C.2 below
  updated to match.
- **`pages-deploy.yml`** — the stage step's `cp -r data assets _site/` is now `cp -r assets
  _site/`; the old line would fail the deploy now that `data/` is gone.
- **The 2026-09-22 ops fix (daily-cards dispatch retry on transient GitHub API errors) is now
...
A single-page, public, static website hosted on GitHub Pages (v3.0 — "Weeks is the site"). It
is a life-in-weeks chart for J and B — one combined 90-year grid, one square per week, headed
by a living "mind" — a small bottle of light, glowing and dimming on a slow breathing cycle, on
repeat — and the current date, always Asia/Hong_Kong. Two themes: a soft pink daytime default
and a warm-charcoal dark theme that owns 17:00–06:00 HKT (v1.29). Zero backend. Zero
dependencies. Zero personal data (about the owner — see §2.1). There are no daily cards
anymore: the Journal card and everything that fed it were retired in v3.0 (see the v3.0
changelog entry above); the page is computed fresh from today's HKT date on every load and
every resumed visit, and changes only when a human pushes to `main`.
...
1. **PII invariant — protects the site owner and his wife, not the ideas being discussed.**
   ...
   audit files, README. This does **not** apply to public figures cited by design:
   attributions like `— after Seneca`, `— after Bill Perkins`, etc. (per §5) are intentional
   and required, not a PII violation — the site is public, and quoting/citing public thinkers
   was the design intent of the now-retired Anchor cards (v1.39) and remains the rule for any
   future content that cites one. The pink theme is still called `blossom`
...
│   └── favicon.svg
├── scripts/
│   └── verify.mjs          # stage-gated verification harness (v3.0: generate-daily.mjs retired)
├── .github/workflows/
│   └── pages-deploy.yml    # v3.0: daily.yml/watchdog.yml retired along with the daily pipeline
├── audits/
│   ├── build-log.md        # heartbeat log
│   ├── decisions.md        # ADR-lite decision log
│   ├── CONTENT-REVIEW.md   # Stage 3: all cards grouped by attribution, QA flags inline; v3.0: retirement note
│   └── stage-N-audit.md    # one per stage + FINAL-AUDIT.md
├── CLAUDE.md               # created in Stage 0: invariants summary + pointer here
├── BUILD-PLAN.md           # this file
├── .nojekyll
├── .gitignore              # .claude/ only (added v1.37 -- agent-worktree tooling state, never shipped)
└── README.md
```

*(v3.0 retired the `data/` directory entirely — `data/cards.json` and `data/daily.json` no
longer exist; historical content survives in git history, at the commit immediately before
this round's, and in `audits/CONTENT-REVIEW.md`.)*
...
1. **Display / card voice:** `Fraunces` (variable, optical size on, italic used for the wordmark) — headings, card body text, wordmark. Literary and warm: the cards are book-derived wisdom, so the type is bookish.
...
On mobile, the header through the Weeks heading and both people's stat rows must fit one
viewport height (`100svh`, `vh` fallback) without scrolling; the grid and epigraph below them
are a deliberate, expected scroll, not an overflow fallback (v3.0 — Weeks is now the sole page
content, so the one-screen rule covers only the part of Weeks the app opens on, not the full
scrollable grid; before that, v1.39 had already re-scoped the same rule once, from the whole
page down to just the Journal card, when Weeks first merged in below it). If a very small
viewport or large system font forces the heading/stats themselves to overflow, allow the page
to scroll further rather than clipping content — never clip.
...
4b. **Pre-09:00 HKT focus mode — retired in v1.39, slot kept (not renumbered) so `item 4b`/`§4.5.4b` cross-references elsewhere in this file stay valid** (item 3, above, points here twice by name — "item 4b below for the focus-mode toggle that also went with it" and "its focus-mode UI at item 4b below" — the same live-reference situation as items 4a and 4c, which item 3 also points to; the v1.39 audit corrected an earlier claim in `decisions.md` that no such pointer existed). Through v1.38: `lib.mjs`'s `isFocusWindowHKT(now)` (HKT hour < 9) gated `renderToday`'s output — inside the window, only the Journal card rendered, followed by a `.reveal-rest` disclosure button (`aria-expanded`/`aria-controls`, 44px tap target, mono pill styling matching the staleness chip/theme toggle) and a `#cards-more` wrapper (`hidden` by default) holding Anchor (held Anchor/Kenya through v1.37, before Kenya's v1.38 retirement); outside the window, both cards rendered flat with no button. Owner request (same round as Anchor's own retirement): "remove the morning hiding function." Removed entirely — `app.js`'s `paintFocusedToday`/`windowMode`/`paintedWindowMode` and the `winMode`/focus branching inside `renderToday`; `lib.mjs`'s `isFocusWindowHKT`; `styles.css`'s `.reveal-rest`/`#cards-more` rules and the desktop `#cards.focus` override. Journal now always renders flat, exactly as item 3 describes, with no button and no wrapper at any hour. Full detail in `audits/decisions.md`'s v1.39 entry.
...
**Retired in v3.0** along with the Journal card and the daily pipeline that fed it — there is
no content library left to specify. The sub-sections below (5.1–5.3) are kept as history, not
current spec; the 1825-entry journal pool they describe survives in git history, at the commit
immediately before this round's, and in `audits/CONTENT-REVIEW.md`.
...
*(`anchors` — added v1.0, expanded 129 -> 365 across a 10-category taxonomy in v1.14 (`stoic` 55, `buddhist` 55, `taoist` 25, `impermanence` 35, `attention` 35, `relationships` 30, `growth` 30, `money` 25, `voices` 40, `grounding` 35) — retired in v1.39, owner: "cut out the anchor"; historical content in `audits/CONTENT-REVIEW.md`.)*
...
1. One idea per card; concrete over abstract. (The specific word cap lives with each surviving card type's own rule — Journal's is rule 5's ≤25 words. Anchor's ≤40-word cap retired with the pool itself in v1.39 — see rule 8.)
2. Original paraphrase only; zero quotation-mark glyphs per invariant 2 (ASCII apostrophes for contractions/possessives are fine). (Attribution style `— after <thinker>` / `— core principle` was Anchor's own convention, retired with the pool in v1.39 — see rule 8; Journal, per rule 5, has never carried attribution.)
...
6. **Values — retired in v1.37 (owner: "we rarely refer to it"), rule slot kept so §5.3.6 cross-references stay valid.** Through v1.36: `{ "name", "essence" (≤ 14 words), "behaviour" (≤ 16 words, observable — something a camera could see) }`, 5 entries in `data/values.json`, generic-safe (no personal references to the owner), cut from 10 to 5 in v1.2 per live human feedback. Removed entirely — `data/values.json` (deleted), `index.html`/`app.js`/`styles.css`'s Values-tab UI, and `sw.js`'s offline shell. Full detail in `audits/decisions.md`'s v1.37 entry.
7. **Attribution-confidence rule — retired in v1.39 along with Anchor, the only card type it ever governed; rule slot kept for consistency with rules 6/8/9/10/11's established treatment.** Through v1.38: use a person-named attribution (`— after X`) only when you are confident the specific idea is centrally/traditionally X's (e.g. Epictetus/Seneca/Marcus Aurelius for Stoic control-of-response ideas, Bill Perkins for Die With Zero's core thesis, Dweck for growth-mindset framing, Carnegie for the specific relationship tactics from *How to Win Friends*, Housel for invisible-wealth/avoid-ruin framing, Newport for deep-work framing) — otherwise demote to tradition-level attribution (`— Stoic tradition`, `— growth-mindset research`, `— core principle`) rather than guess at a specific person. Journal, per rule 5, has never carried attribution, so there is nothing left for this rule to govern. Full detail in `audits/decisions.md`'s v1.39 entry.
8. **Anchor authoring process — retired in v1.39 (the anchors pool itself retired), rule slot kept (not renumbered) so `rule 8`/`§5.3.8` cross-references stay valid** — three live pointers land here, all written or edited in v1.39's own sweep: §4.5 item 3's "see item 8 in §5.3 for the authoring-side retirement," and rules 1 and 2 above, which each defer their retired Anchor-specific clause with "see rule 8." (Rules 7 and 9's slots are kept for consistency instead: their only pointers — Stage 3's task list for `§5.3.7`, the v1.3 changelog for `§5.3.9` — live in this file's deliberately-frozen historical text, not in live spec prose.) Through v1.38: anchors were written in six batches (one per category, extending each category's 3 seed cards to its full count); after each batch, the normal mechanical self-review (word caps, quote marks, banned phrases) ran, **and then** a second, independent review pass per §10 Stage 3's content-QA step, before moving to the next batch. Historical content preserved, retired-labeled, in `audits/CONTENT-REVIEW.md`; full detail in `audits/decisions.md`'s v1.39 entry.
9. **`voices` category — retired in v1.39 along with the rest of anchors, rule slot kept for consistency with rules 6/7/8/10/11's established treatment.** Through v1.38: a 7th anchor category (added v1.3), 9 cards (3 each) [later expanded to 40 in the v1.14 taxonomy rewrite — see §5.2], for named living/recent public figures the owner's household specifically found inspiring, so their thinking surfaced periodically via the same rotation — not a separate subsystem, just entries in the same `anchors` pool. Extra bar beyond the (also now-retired) rules 1–8: (a) any figure who had held significant political office was scoped strictly to personal-character themes (grief, resilience, self-belief, service), never their office, party, policies, or elections; (b) rule 2's "no verbatim quotes" was enforced against the *spirit*, not just quote-mark glyphs; (c) the independent second-pass review (rule 8) ran with an explicit prompt to check attribution-confidence, political neutrality, and closeness-to-source. Historical content preserved, retired-labeled, in `audits/CONTENT-REVIEW.md`; full detail in `audits/decisions.md`'s v1.39 entry.
...
12. **Card-type scope, current as of v1.39:** exactly one card type ships — Journal (rule 5). Every retired card type's own rules stay in their original numbered slots as stubs (rules 6, 7, 8, 9, 10, 11, for Values/attribution-confidence/anchor-authoring/voices/wordOfDay/kenya respectively), not renumbered, matching this section's established convention; rules 1–4 apply generically to whatever card types exist at a given time and were edited in place (not stubbed) when Anchor retired, since a platitude ban or a quote-glyph ban doesn't stop meaning anything just because one card type is gone.

---

## §6 — Data & logic

**§6.1/6.2/6.3 all retired in v3.0** along with the daily pipeline, `daily.json`, and the
rotation engine — Weeks needs none of this, since it is computed directly from today's HKT
date via `lib.mjs`'s HKT date/time and life-in-weeks helpers (kept, unchanged), not picked from
a content pool. Kept below as history.

### 6.1 `daily.json` schema (retired v3.0)
...
```
The id is always present — unlike the retired `fresh` field (v1.0–v1.9), there is
nothing here that can come back `null` or need URL/source validation, since it is a
deterministic pick from the app's own pool (§6.2), not a value pulled from any external
source. (`wordId` was a fourth id here through v1.34, retired in v1.35; `kenyaId` was a third
id through v1.37, retired in v1.38; `anchorId` was the second id through v1.38, retired in
v1.39. Each ordinal counts the ids that existed alongside it at the time, the same convention
this note has used since v1.35.)

### 6.2 Deterministic rotation (retired v3.0; was in `lib.mjs`, shared browser + node)
...
## §7 — Daily pipeline (retired v3.0)

**Retired in v3.0** along with the Journal card it fed — `scripts/generate-daily.mjs` is
deleted, and there is nothing left in the repo that stamps a daily pick. Kept below as history.
...
feedback disliked the Fresh card; its v1.10 replacement (Word of the Day, itself later retired
...
today's one pick (a second, Anchor, `anchorId`, was retired in v1.39; a third, Kenya,
`kenyaId`, was added in v1.15 and retired again in v1.38):

1. Compute `dateHKT`, `dayNumber` via `lib.mjs`; pick `journalId`
   (via the same deterministic `pickIndex` rotation, §6.2 — no fetch, no external
   dependency).
2. Write `data/daily.json` (§6.1).
3. Idempotent and safe to re-run any number of times same-day — the pick is a pure function
   of `dateHKT`, so a re-run just rewrites `generatedAtISO` and reproduces the same id.
...
**v1.35 note:** Word of the Day (added v1.10) was retired — owner: "not helpful" — dropping
`wordId` from this script and from `daily.json`'s schema. The pipeline was already this simple
for the Fresh-card reason above; this just removes one more picked id, not any machinery.

**v1.39 note:** Anchor (present since v1.0) was retired — owner: "cut out the anchor" —
dropping `anchorId` from this script and from `daily.json`'s schema. The pipeline now stamps a
single id, `journalId`.
...
automated pipeline is alive, independent of which specific cards it's stamping.

---

## §8 — Workflows (copy these, adjusted per this v1.1, then validate)

**§8.1/8.2 retired in v3.0** — `daily.yml` and `watchdog.yml` are both deleted; only
`pages-deploy.yml` (on push to `main`, unchanged in trigger) remains. The YAML blocks below stay
as history.

### 8.1 `.github/workflows/daily.yml` (retired v3.0)
...
### 8.2 `.github/workflows/watchdog.yml` (retired v3.0)
...
**LOOP-A (daily refresh) and LOOP-C (dead-man's switch) are retired in v3.0** along with the
daily pipeline and the watchdog — there is no scheduled content to refresh or watch for
staleness anymore. LOOP-B is moot with them (nothing left to hang). Only `pages-deploy.yml`
remains, on push to `main`.

| Loop | Mechanism | Catches |
|---|---|---|
| **LOOP-A** daily refresh — *retired v3.0* | ~~Actions cron 04:56 HKT + `workflow_dispatch`~~ | ~~the core auto-run~~ |
| **LOOP-B** stuck-build killer — *retired v3.0, nothing left to hang* | ~~`timeout-minutes: 10` + `concurrency.cancel-in-progress`~~ | ~~any hung/overlapping run~~ |
| **LOOP-C** dead-man's switch — *retired v3.0* | ~~watchdog 09:00 HKT → checks the *live* site, not just git → GitHub issue (deduped) + email~~ | ~~silent failures LOOP-B can't see~~ |
| **LOOP-D** graceful degrade — v3.0: Weeks computes fresh from today's HKT date, no fetch, nothing to degrade from | client renders Weeks directly from `lib.mjs`; a build failure shows the `.weeks-error` state (§4.5 item 7) | the page never shows a blank screen on a real failure, it shows an honest error |

Design consequence (v3.0): the page has nothing left that can go stale, since it never fetches
anything — the only production loop left is a human pushing to `main`, which `pages-deploy.yml`
deploys after `verify.mjs all` gates it green.
...
## Appendix B — Reference rotation implementation (retired v3.0; kept as history)

**Retired in v3.0** along with the daily pipeline and the Journal content it rotated —
`pickIndex`/`xmur3`/`mulberry32` and the rest of this rotation engine no longer exist in
`lib.mjs`. `hktDateString`/`hktDayNumber`/`hktDateParts` (shown in the code block below) are
the only functions here still shipped; they're kept because `weeksLived()` and the date line
depend on them. Kept below as history, not current spec.
...
```json
{
  "name": "Mindset",
  "short_name": "Mindset",
  "description": "A daily journal prompt and a life-in-weeks chart, every morning at 05:00 HKT.",
  "display": "standalone",
  "start_url": "./",
  "scope": "./",
