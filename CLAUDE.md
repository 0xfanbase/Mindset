...
2. No verbatim quotes: zero quotation-mark glyphs in user-facing copy (the Weeks epigraph; ASCII apostrophes in contractions are fine). Attribution only, never excerpts.
...
11. Mobile-first: base CSS is mobile, `min-width` queries only (`max-width` banned), relative URLs only, the Weeks heading and both people's stat rows complete above the fold at 390×844 (the grid below them scrolls — v3.0 re-scoping, logged in decisions.md), `svh` + safe-area insets, installable PWA.
