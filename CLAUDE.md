...
2. No verbatim quotes: zero quotation-mark glyphs in user-facing copy (the epigraph; ASCII apostrophes in contractions are fine). Attribution only, never excerpts.
3. Zero runtime deps: vanilla HTML/CSS/JS, no build step, no npm install, `localStorage` keys `mindset.*` only (none remain). Dark theme only (v4.0).
4. Node ≥ 20 built-ins only for scripts.
...
7. WCAG AA contrast, keyboard focus, aria roles on the view tabs and person switch, 44px tap targets, semantic landmarks.
8. Every date is Asia/Hong_Kong via `Intl.DateTimeFormat` with explicit `timeZone` — never bare local time.
9. `<meta name="robots" content="noindex">`.
10. No force-push, no history rewrites of pushed commits. `git pull --rebase` on unpushed local commits is fine.
11. Mobile-first: base CSS is mobile, `min-width` queries only (`max-width` banned), relative URLs only, the hero (heading, person switch, number, both bars) complete above the fold at 390×844 (the grid scrolls — v4.0 re-scoping, logged in decisions.md), `svh` + safe-area insets, installable PWA.
